...
class WasmContract:
    ...
    * Deploy a new smart contract using :py:meth:`Contract.deploy`
    ...
    def transact(self, transaction=None):
        """
        Execute a contract function call using the `platon_sendTransaction`
        interface.
...
class ContractConstructor:
    ...
    """
    def __init__(self, web3, abi, bytecode,vmtype, *args, **kwargs):
        self.web3 = web3
        self.abi = abi
    ...
    def _encode_data_in_transaction(self, *args, **kwargs):
        ...
        if constructor_abi:
            ...
            arguments = merge_args_and_kwargs(constructor_abi, args, kwargs)
            data = add_0x_prefix(
                encode_abi(self.web3, constructor_abi, arguments, self.vmtype, self.bytecode, self.abi)
            )
        else:
