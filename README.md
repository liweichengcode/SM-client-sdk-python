...
​       client-sdk-python采用两种加密算法模式，ECDSA-SHA3算法和国密SM2-SM3算法。ECDSA和SM2都是基于椭圆曲线离散对数问题，属于椭圆曲线密码算法，其密码的单位比特强度要高于其他公钥体制。SHA3和SM3都是64位加密散列算法，生成哈希值。在  client-sdk-python加密体系中，可选择'ECDSA'模式或‘SM’模式，分别代表ECDSA-SHA3加密算法体系和国密SM2-SM3算法体系。
...
#### 2 签名
 ...
 在 client-sdk-python中，可通过account.signTransaction(txn,private_key=privatekey,net_type=TESTNETHRP,mode='SM')
...
​     在 client-sdk-python中，可以对任意信息的哈希值进行签名，
...
​    在 client-sdk-python中，可对已签名的签名结果进行验证，验证是否签名成功。以国密加密签名的验证方式为例：
...
#### 4 密钥保存（keystore）
   ...
   在 client-sdk-python中，可对密钥使用AES加密方法进行保存。以免用户丢失或泄露密钥信息。通过account.encrypt()和account.decrypt()对密钥进行加密保存和解密获取。
...
```python
{'address': {'mainnet': 'lat1jkak6wudnp3289dav8wcvpkkdu28fqf30m3vzh', 'testnet': 'lax1jkak6wudnp3289dav8wcvpkkdu28fqf3q7rrvc'},
...
即lat1jkak6wudnp3289dav8wcvpkkdu28fqf30m3vzh.json。每一个keystore的json文件都是以主网地址命名的，具有独一性。
...
>>>prikey.hex()
'0x6d86da8039e20aca51e7f9ef84eb2d4d7e921907ff3833e7ee3a59940c2e2833'
```
